...
Although the MPS formalism excels at describing one-dimensional systems, it struggles at capturing long-ranged correlations, due to how it is constructed.
...
\end{figure}
According to eq. \eqref{eq:corrfunction}, correlation functions are given by a linear combination of exponential functions in the MPS description. Therefore, matrix product states struggle at fully capturing long-ranged correlations, such as the superfluid single-particle correlations. Single-particle correlations decay exponentially for the Mott-insulator, while they for superfluids decay following a power-law 
\begin{equation}
	\braket{\hat{a}_{i}^{\dag} \hat{a}_{j}} \sim |i - j|^{-K_b /2} \; ,
	\label{eq:superfluidCorrelation}
\end{equation}
where $K_b$ is the Tomonaga-Luttinger parameter \cite{characPhases}. For short distances eq. \eqref{eq:corrfunction} is able to accurately approximate a power-law, however, only the slowest exponential decay will survive as distances grow larger. Hence, the correlation turns into a pure exponential decay with $\xi = -1/ \ln \lambda$, where $\lambda$ is the largest eigenvalue of $\hat{E}$.\\
...
Figures \ref{fig:DensityMatrices}(a) and \ref{fig:DensityMatrices}(c) show the density matrix plotted for the superfluid and the Mott-insulator limit respectively. In the superfluid limit, long-range correlations are present, which is seen by large off-diagonal elements. In the superfluid phase the energy of the system is minimized by de-localizing the particles across the entire lattice. However, the open boundary conditions causes a depletion of particles at the outer sites. This in turn results in the decrease of single-particle correlations near the boundary.

Figure \ref{fig:DensityMatrices}(d) displays the single-particle correlation function as function of distance from the diagonal. In the $U/J = 0$ limit, the correlation function clearly does not exhibit a power-law behavior. The reason for this is two-fold: First, correlation functions are given by an exponential series in the MPS representation, which causes a poor description of very long-ranged correlation. Secondly, in the limit of large systems and negligible interactions the superfluid spectrum becomes gapless, whereby the ground state does not strictly follow an area law anymore. Therefore, the DMRG algorithm must search the entire Hilbert space for the ground state candidates resulting in a much worse convergence. 


In the Mott-insulator limit no interaction takes place between sites, and the correlation length is zero. Hence, the density matrix contains only diagonal elements of equal magnitude. The density matrix shown in figure \ref{fig:DensityMatrices}(c) shows some off-diagonal elements of non-zero magnitude, since the system is not a pure Mott-Insulator at $U/J = 12$. Nevertheless, the correlations are well described by only a single exponential function, as seen by the figure.

Finally, figure \ref{fig:DensityMatrices}(b) illustrates the density matrix for a system with $U/J = 0.5$, which is still in the superfluid phase but has finite interactions. The particles are not completely de-localized, which is noticeable from the visible diagonal. Plotting the correlation function reveals the best approximation being a power-law, confirming the system is indeed a superfluid. 
